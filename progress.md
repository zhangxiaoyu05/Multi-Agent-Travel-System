...
>
> 最后更新：2026-08-05（Phase 22-续-2 行程定制预检——修复 LLM 误判导致路由卡客服）
...
| 2026-08-05 | **Phase 22-续-3：侧边栏对话时间修复**——① 问题：前端左侧对话列表的时间始终停留在创建/标题变更时刻，不随新消息更新；② 根因：`services/memory.py` 的 `save_message()` 只插入 `chat_messages` 表，未同步更新 `conversations.updated_at`（MySQL `ON UPDATE CURRENT_TIMESTAMP` 只在本表行更新时触发）；③ 修复：`save_message()` 插入消息后追加 `UPDATE conversations SET updated_at = CURRENT_TIMESTAMP`；④ `services/memory.py` +5 行；⑤ 255 测试全部通过 | ✅ |
| 2026-08-05 | **Phase 22-续-2：行程定制预检——修复 LLM 误判导致路由卡客服**：① 问题——用户消息"我想去拉萨，一个人，预算5000元，8-15到"被 qwen-plus LLM 意图分类器反复误判为 service（客服），导致 `journey_stage` 始终停留在 discovery，永远路由到客服 Agent 而非行程定制 Agent；② 根因——LLM 在对话历史惯性下产生分类偏差（客服回复→下一轮延续客服）；③ 修复——在 `intent_router.py` 新增行程定制预检过滤器：8 个正向正则模式（独立"定制"请求、设计/规划/安排行程、目的地改为/换成、目的地+预算/天数/日期多要素组合、目的地+玩/旅游结尾、独立"去X天"、目的地+预算+日期三要素、独立"我想去X"）+ 4 个 FAQ 排除模式（签证/证件/边防证、流程/步骤询问、怎么/如何定制、需要签证/材料）；④ `_has_trip_planning_intent()` 函数——先检查排除模式再检查正向模式，FAQ 类行程问题不误触 planner；⑤ `_prefilter_user_message()` 新增优先级 3——行程定制预检返回 `planner=0.85` + `journey_stage="planning"`，跳过 LLM 调用；⑥ ⑤-b 模式添加 `(?![一-鿿])` 负向前瞻——防止"我想去成都怎么走"被贪婪匹配误判；⑦ `_TRIP_PLANNING_PATTERNS` 正则允许量词和关键词之间有修饰内容（如"帮我安排一个3天的行程"中"3天的"）；⑧ 支持 `\d+天` 格式（如"去成都5天"，不仅限于"几天"）；⑨ 测试：新增 10 个预检单元测试（TestTripPlanningPrefilter）+ 修复 2 个预存测试（空文本期望值 1.0→0.85）；⑩ Chrome DevTools E2E 浏览器验证：7 场景全部通过——行程定制(85%)全链路接力→销售(85%)→negotiation挽留；⑪ 2 文件修改（`graph/nodes/intent_router.py` + `tests/test_router.py`），255 测试全部通过 | ✅ |① `revision_decision` accept+journey_stage 变更→`route_decision`（同轮交接，不再等待下一轮用户消息）；② `intent_scorer→route_decision` 新边（`builder.py`）；③ trip_planner 新增 `_is_confirm_signal()` 快速确认检测——16 个确认/修订模式，确认时跳过不必要的 draft 重生成；④ 新增 `process_payment` 假支付工具（`mock_sales.py` + `mcp_tools.py` 包装器）；⑤ sales_agent 注册 `process_payment` + WON 检测重构——**仅支付成功触发**（`process_payment` 工具结果 / "支付成功"文字），`create_order` 单独不再误判 WON；⑥ `_determine_next_stage` 新增 `tool_results` 参数，优先检测 `process_payment` 工具结果；⑦ sales_closing Prompt 重写支付流程引导（"回复「支付」完成付款"）；⑧ trip_planner Prompt 更新——移除手动邀请文字（系统自动交接）；⑨ 测试更新：`test_graph.py` 移除 `operations_handoff` + 新增 `route_decision` 边测试；`test_sales.py` WON 检测适配新逻辑（3 用例）；⑩ E2E 浏览器验证：定制确认→销售自动接管（"💰 销售咨询" 标签同轮出现）；⑪ 路由/支付/确认信号单元测试全部通过（98 测试） | ✅ |
