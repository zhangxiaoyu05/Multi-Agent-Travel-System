...
| 2026-07-31 | Chrome E2E 测试 + 两项修复：① 意图路由器 v2——整合对话历史 + current_branch 上下文（intent_router.py 注入最近 6 条消息、prompt 新增对话延续规则、route_decision 新增 0.25 惯性偏向阈值），解决跟进消息误路由问题（"8月20日出发，预算3000元" 从 customer_service → trip_planner）；② Docker 镜像修复——Dockerfile 配置阿里云 apt/pip 镜像源（mirrors.aliyun.com），重建后容器 healthy（python-jose 缺失问题解决）。177 测试全通过 | ✅ |
