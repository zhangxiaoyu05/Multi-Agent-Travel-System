...
使用方式：
    ...
    router_llm = get_router_llm()   # 轻量模型（qwen-turbo），用于意图路由
    agent_llm = get_agent_llm()     # 强模型（qwen3-max），用于内容生成
"""
...
def _get_api_key() -> str:
    """获取 API Key（运行时从环境变量读取，不缓存以便 .env 变更生效）"""
